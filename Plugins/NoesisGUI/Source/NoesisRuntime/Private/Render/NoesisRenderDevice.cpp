...
public:
	...
	{
#if UE_VERSION_NEWER_THAN_OR_EQUAL(5, 6, 0) && UE_WITH_PSO_PRECACHING
	...
	for (int32 PermutationIdx = 0; PermutationIdx < FNoesisPS::FPermutationDomain::PermutationCount; PermutationIdx++)
	{
		...
		for (bool SinglePassStereo : { false, true })
		{
			for (bool IsLinearColor : { false, true })
			{
				...
				{
					AddPSOInitializers(ShaderMap, SceneTexturesConfig, ShaderCode, StateCode, PixelShader.GetPixelShader(), IsLinearColor, SinglePassStereo, GammaCorrection, PSOCollectorIndex, PSOInitializers);
				}
			}
	...
	if (Material.IsUIMaterial() || Material.IsPostProcessMaterial())
	{
			...
			for (int32 PermutationIdx = 0; PermutationIdx < FNoesisCustomEffectPS::FPermutationDomain::PermutationCount; PermutationIdx++)
			{
				...
				for (bool SinglePassStereo : { false, true })
				{
					for (bool IsLinearColor : { false, true })
					{
						...
						for (uint32 StateCode = 0; StateCode < 256; StateCode++)
						{
							AddPSOInitializers(GlobalShaderMap, SceneTexturesConfig, Noesis::Shader::Custom_Effect, StateCode, PixelShader.GetPixelShader(), IsLinearColor, SinglePassStereo, GammaCorrection, PSOCollectorIndex, PSOInitializers);
						}
					}
		...
		else if (Material.IsUIMaterial())
		{
			for (int32 PermutationIdx = 0; PermutationIdx < FNoesisMaterialPS::FPermutationDomain::PermutationCount; PermutationIdx++)
			{
				...
				const bool IsLinearColor = Permutation.Get<FNoesisMaterialPS::FLinearColor>();
				...
				for (bool SinglePassStereo : { false, true })
				{
					for (uint32 StateCode = 0; StateCode < 256; StateCode++)
					{
						AddPSOInitializers(GlobalShaderMap, SceneTexturesConfig, ShaderCode, StateCode, PixelShader.GetPixelShader(), IsLinearColor, SinglePassStereo, GammaCorrection, PSOCollectorIndex, PSOInitializers);
					}
