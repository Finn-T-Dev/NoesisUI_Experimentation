...
#include <NsGui/ComparisonLogic.h>
...
void ChangePropertyAction::Invoke(BaseComponent*)
{
    ...
    if (GetAssociatedObject() != 0 && target != 0)
    {
        ...
        if (mTypeProperty != 0 || mDependencyProperty != 0)
        {
            Noesis::BindingOperations::EnsureBindingValue(this, ValueProperty);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void ChangePropertyAction::OnTargetChanged(Noesis::BaseComponent*, Noesis::BaseComponent*)
{
    // reset cached property because it is no longer valid
    mTypeProperty = 0;
...
void ChangePropertyAction::UpdateConvertedValue()
{
    ...
    const Noesis::Type* valueType = value != 0 ? Noesis::ComparisonLogic::ExtractBoxedType(value) : 0;
...
NS_IMPLEMENT_REFLECTION(ChangePropertyAction, "Noesis.Interactivity.ChangePropertyAction")
{
  #ifdef NS_HAVE_STUDIO
    NsProp("PropertyName", &ChangePropertyAction::GetPropertyName, &ChangePropertyAction::SetPropertyName)
        .Meta<Noesis::StudioOrder>(1)
        .Meta<Noesis::StudioProp>("AttributeProperty",
            [](Noesis::BaseComponent* instance) -> Noesis::BaseComponent*
            {
                ChangePropertyAction* action = Noesis::DynamicCast<ChangePropertyAction*>(instance);
                return action ? action->GetTarget() : nullptr;
            });
    NsProp("Value", &ChangePropertyAction::GetValue, &ChangePropertyAction::SetValue)
        .Meta<Noesis::StudioOrder>(2);
    NsProp("Duration", &ChangePropertyAction::GetDuration, &ChangePropertyAction::SetDuration)
        .Meta<Noesis::StudioOrder>(-1);
    NsProp("Increment", &ChangePropertyAction::GetIncrement, &ChangePropertyAction::SetIncrement)
        .Meta<Noesis::StudioOrder>(-1);
  #endif
