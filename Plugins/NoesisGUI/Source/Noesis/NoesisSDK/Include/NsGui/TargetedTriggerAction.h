////////////////////////////////////////////////////////////////////////////////////////////////////
// NoesisGUI - http://www.noesisengine.com
// Copyright (c) 2013 Noesis Technologies S.L. All Rights Reserved.
////////////////////////////////////////////////////////////////////////////////////////////////////


#ifndef __GUI_TARGETEDTRIGGERACTION_H__
#define __GUI_TARGETEDTRIGGERACTION_H__


#include <NsCore/Noesis.h>
#include <NsGui/TriggerActionBase.h>
#include <NsGui/InteractivityApi.h>


namespace Noesis
{

////////////////////////////////////////////////////////////////////////////////////////////////////
/// Represents an action that can be targeted to affect an object other than its *AssociatedObject*.
////////////////////////////////////////////////////////////////////////////////////////////////////
class NS_GUI_INTERACTIVITY_API TargetedTriggerAction: public TriggerActionBase
{
public:
    virtual ~TargetedTriggerAction() = 0;

    /// Gets or sets the target object
    //@{
    BaseComponent* GetTargetObject() const;
    void SetTargetObject(BaseComponent* target);
    //@}

    /// Gets or sets the name of the object this action targets
    //@{
    const char* GetTargetName() const;
    void SetTargetName(const char* name);
    //@}

    // Hides Freezable methods for convenience
    //@{
    Ptr<TargetedTriggerAction> Clone() const;
    Ptr<TargetedTriggerAction> CloneCurrentValue() const;
    //@}

public:
    static const DependencyProperty* TargetObjectProperty;
    static const DependencyProperty* TargetNameProperty;

protected:
    TargetedTriggerAction(const TypeClass* targetType);

    // For internal use only
    //@{
    TargetedTriggerAction();
    void SetTargetType(const TypeClass* targetType);
    //@}

    // If TargetObject is not set, the Target will look for the object specified by TargetName
    // If TargetName element cannot be found, the Target will default to the AssociatedObject
    BaseComponent* GetTarget() const;

    // Called when the target changes
    virtual void OnTargetChangedImpl(BaseComponent* oldTarget, BaseComponent* newTarget);

    // From AttachableObject
    //@{
    void OnAttached() override;
    void OnDetaching() override;
    //@}

private:
    BaseComponent* GetTargetNameResolver() const;

    void UpdateTarget(DependencyObject* associatedObject);

    void RegisterTarget();
    void UnregisterTarget();
    void OnTargetDestroyed(DependencyObject* d);

private:
    const TypeClass* mTargetType;
    BaseComponent* mTarget;

    static const DependencyProperty* TargetNameResolverProperty;

    NS_DECLARE_REFLECTION(TargetedTriggerAction, TriggerActionBase)
};

////////////////////////////////////////////////////////////////////////////////////////////////////
/// Templatized base class for TargetedTriggerAction.
////////////////////////////////////////////////////////////////////////////////////////////////////
template<class T>
class TargetedTriggerActionT: public TargetedTriggerAction
{
protected:
    inline TargetedTriggerActionT(): TargetedTriggerAction(TypeOf<T>()) { }
    inline T* GetTarget() const { return static_cast<T*>(ParentClass::GetTarget()); }

    // Called when the target changes
    virtual void OnTargetChanged(T* oldTarget, T* newTarget)
    {
        NS_UNUSED(oldTarget, newTarget);
    }

private:
    // From TargetedTriggerAction
    //@{
    void OnTargetChangedImpl(BaseComponent* oldTarget, BaseComponent* newTarget) final
    {
        OnTargetChanged(static_cast<T*>(oldTarget), static_cast<T*>(newTarget));
    }
    //@}

    NS_IMPLEMENT_INLINE_REFLECTION_(TargetedTriggerActionT, TargetedTriggerAction)
};

}


#endif
