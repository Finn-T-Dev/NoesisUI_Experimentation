...
{
public:
    typedef void (*CreateCallback)(RenderDevice* device, void* user);
    typedef void (*DestroyCallback)(RenderDevice* device, void* user);
    typedef Texture* (*TextureRenderCallback)(RenderDevice* device, uint32_t width, uint32_t height,
        void* user);

    DynamicTextureSource(uint32_t width, uint32_t height, CreateCallback create,
        DestroyCallback destroy, TextureRenderCallback render, void* user);
    ...
    /// Invalidates and updates the render of the dynamic texture
    void Update();
...
private:
    ...
    TextureRenderCallback mRenderCallback;
    void* mUser;
