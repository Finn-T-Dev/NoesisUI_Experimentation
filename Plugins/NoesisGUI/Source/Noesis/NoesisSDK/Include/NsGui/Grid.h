...
public:
    ...
    //@{
    static int32_t GetColumn(const DependencyObject* element);
    static void SetColumn(DependencyObject* element, int32_t column);
    //@}

    /// Gets or sets a value that indicates the number of columns that child spans within a Grid
    //@{
    static int32_t GetColumnSpan(const DependencyObject* element);
    static void SetColumnSpan(DependencyObject* element, int32_t columnSpan);
    //@}
    ...
    //@{
    static int32_t GetRow(const DependencyObject* element);
    static void SetRow(DependencyObject* element, int32_t row);
    //@}

    /// Gets or sets a value that indicates the number of rows that child spans within a Grid
    //@{
    static int32_t GetRowSpan(const DependencyObject* element);
    static void SetRowSpan(DependencyObject* element, int32_t rowSpan);
    //@}
...
private:
    ...
    void GetColumnInfo(const UIElement* child, uint32_t numCols,
        int32_t& col, int32_t& colSpan) const;
    void GetRowInfo(const UIElement* child, uint32_t numRows,
        int32_t& row, int32_t& rowSpan) const;
    //@}
    ...
    void UpdateSizes(const DefinitionCacheVector& defVec, uint32_t index, int32_t span,
    ...
    void DistributeSize(uint32_t index, int32_t span, float size,
    ...
    float GetSpanSize(const DefinitionCacheVector& defVec, const FloatVector& sizes,
        float constraint, uint32_t index, int32_t span, bool defsSolved) const;

    // Indicates if all cells in the span are auto
    void GetAllAutoSpanAndOrder(const DefinitionCacheVector& defVec, uint32_t index, int32_t span,
